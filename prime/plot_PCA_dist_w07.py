...
print("this script plots fig as Fig.6b in wilson2007")
...
def PC_dist(theCoup):
    ...
    for t in range(real_begin, real_end, 50):
        #print(theCoup, t)
        distls=[]
        xtp=loadtxt('data/pca_onnet%d_time%d_%d.txt'%(theCoup,t,t+50))
...
def odor_PC_dist():
    ...
    for t in range(real_begin, real_end, 50):
        #print(theCoup, t)
        distls=[]
        xtp=loadtxt('data/pca_offnet_time%d_%d.txt'%(t,t+50))
        for i in idx: distls.append(norm(xtp[i[0]]-xtp[i[1]], 2))
        distls = sort(distls)
        dist_t_avg_ls.append(avg(distls))
        l = len(distls)
...
axvspan(22, 27, facecolor='0.5', alpha=0.5) # 21-27:50-350ms => 100-400ms
show()
savefig('allPC_odor_avg_3qs.jpg');
clf()
