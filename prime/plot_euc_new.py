...
print("\nthis is the new version of plot_euc.py")
print(" --- it computes Euc distance over all trial permutation")
print("time period:", tbgn,tend)

def dist_from_Qcs_to_Qc0_avg_std_ls(c,s, tb,te,td):
    # for given odor (defined by c and s),
    # compute all trials avg&std distance of this odor to canonical odor,
    ...
    for i,j in itertools.product(range(trial_number),range(trial_number)):
        ...
        yls.append(comp_distance_from_sf(c,0,i, c,s,j, tb,te,td))
    return avg(yls), std(yls), yls


def dist_ls_for_a_couple_avg_std(c, tb,te,td):
    # for the given couple (c),
    # compute the distance between (1) each its shifts and (2) its 0 shifts odor
    d_avg, d_std = zeros(shift_number), zeros(shift_number)
    for i,s in enum(shift_list):
        d_avg[i],d_std[i],_ = dist_from_Qcs_to_Qc0_avg_std_ls(c,s,tbgn,tend, td)
    return d_avg, d_std
...
savetxt("euc_ad_avg_odorset0_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_avg0)
'''
savetxt("euc_ad_avg_odorset1_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_avg1)
savetxt("euc_ad_avg_odorset2_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_avg2)
savetxt("euc_ad_avg_odorset3_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_avg3)
savetxt("euc_ad_avg_odorset4_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_avg4)
'''

savetxt("euc_ad_std_odorset0_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_std0)
'''
savetxt("euc_ad_std_odorset1_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_std1)
savetxt("euc_ad_std_odorset2_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_std2)
savetxt("euc_ad_std_odorset3_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_std3)
savetxt("euc_ad_std_odorset4_%d_%d_%d.txt"%(tbgn,tend,tlen), rd_std4)
'''
...
savetxt("euc_od_avg_%d_%d_%d.txt"%(tbgn,tend,tlen), od_avg)
savetxt("euc_od_std_%d_%d_%d.txt"%(tbgn,tend,tlen), od_std)
savetxt("euc_ad_avg_%d_%d_%d.txt"%(tbgn,tend,tlen), ad_avg)
savetxt("euc_ad_std_%d_%d_%d.txt"%(tbgn,tend,tlen), ad_std)
...
savefig("distance_%d_%d_%d_new.jpg"%(tbgn,tend,td))
savefig("distance_%d_%d_%d_new.eps"%(tbgn,tend,td))
#show()
clf()
...
savefig("distance_slope_%d_%d_%d_new.jpg"%(tbgn,tend,td))
savefig("distance_slope_%d_%d_%d_new.eps"%(tbgn,tend,td))
show()
clf()
'''